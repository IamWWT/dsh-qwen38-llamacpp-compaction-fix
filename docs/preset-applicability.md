...
> 但"自动压缩"本身是 preset 提供的,极简模式(`minimal`)没有,插件不会替它装。**
...
- **没有任何自动压缩引擎注册**:插件不组装 `compaction-basic`、不注册 step 钩子,所以
  minimal 里"到期自动压缩"依旧不存在(与官方行为一致)。
...
- **自动压缩只属于 standard / ptc / cordis**;minimal 会话要压缩请用
  `/qwen38-compact`(有损保信息)或 `/qwen38-new-context`(秒级硬重置)。
