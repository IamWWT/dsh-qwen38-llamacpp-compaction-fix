...
dsh web 的设置面板里只有一个入口——**设置 → 插件 → 插件配置** 里的插件卡片
(`Qwen3.8 网关压缩修复`),与内置插件一致;卡片头部可点击展开/收起,保存落定后自动
收起(不再有额外的左侧导航条目)。

界面布局:

- 卡片头部:插件名 + 一句作用域说明;展开后才渲染下面的控件;
- **基础设置**:作用域提示、`/qwen38-compact` / `/qwen38-new-context` 手动命令说明、
  适用模型 ID;三个拨动开关(带“已启用/已停用”文字,不是裸复选框):
